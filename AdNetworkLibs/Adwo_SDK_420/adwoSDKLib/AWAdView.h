...
// SDK版本号的数值表示
#define ADWO_SDK_VERSION                    0x42

// SDK版本号的字符串表示，用于传给页面
#define ADWO_SDK_VERSION_STRING             @"4.2"
...
/**
 * 描述：这个方法用于展示全屏广告，因此仅支持全屏广告模式。可参考全屏展示Demo。
 ...
 * 这边另外要注意的是，当你调用这个接口后，相应的AWAdView对象将会被添加到baseView上，这个操作类似于[baseView addSubview:adView]。因此调用完这个接口后，AWaAdView对象的引用计数会自动加1。若当前的AWAdView对象还没有调用此方法就要被释放，那么只需调用release即可；否则需要调用removeFromSuperview。
*/
- (BOOL)addedToShowFullScreenAd:(UIView*)baseView orientation:(UIInterfaceOrientation)currOrientation;
